...
if ($a == 'add')
{
	...
	for ($i = 1; $i <= $stagescount; $i++) {
        if ($stagescount > 1) {
            // Если у нас тольуо 1 этап. Название этапа можно не указывать
		...
		$rsbr['sbr_cost'] += (isset($rstagecost[$i]) ? $rstagecost[$i] : 0);
	}
...
for($i = 1; $i <= $stagescount; $i++)
{
	$t->assign(array(
		...
		'STAGEADD_FORM_COST' => cot_inputbox(
            ...
            array('class' => 'stagecost', 'size' => '10', 'maxlength' => '100')
        ),
		'STAGEADD_FORM_DAYS' => cot_inputbox(
            'text',
            'rstagedays['.$i.']',
            isset($rstagedays[$i]) ? $rstagedays[$i] : '',
            array('size' => '10', 'maxlength' => '100')
        ),
	));
